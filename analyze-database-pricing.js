  ...
  try {
    ...
    const taxSettings = await prisma.systemSettings.findMany({
      where: {
        OR: [
          ...
          { setting_key: 'service_charge_rate' }
        ]
      }
    });
    ...
    if (taxSettings.length === 0) {
      ...
      taxSettings.forEach(setting => {
        console.log(`  • ${setting.setting_key}: ${setting.setting_value}${setting.data_type === 'DECIMAL' ? '%' : ''}`);
    ...
    const pricingRules = await prisma.dynamicPricingRules.findMany({
      select: {
        id: true,
        name: true,
        priority: true,
