  ...
  try {
    ...
    for (let i = 0; i < hotelRoomsData.length; i++) {
      ...
      try {
          ...
          if (roomData.roomTypeData) {
            roomType = await prisma.roomType.create({
              data: {
                ...
                capacity: roomData.roomTypeData.capacity,
                ...
                extraBedCharge: roomData.roomTypeData.extraBedCharge || 0,
                sizeSqm: roomData.roomTypeData.sizeSqm,
        ...
        console.log(`   💰 ราคา: ฿${roomType.baseRate.toLocaleString()}/คืน | 👥 จำนวน: ${roomType.capacity} คน`);
        successCount++;
    ...
    if (successCount > 0) {
      ...
      roomTypes.forEach((roomType, name) => {
        console.log(`   • ${name}: ฿${roomType.baseRate.toLocaleString()}/คืน (จำนวน: ${roomType.capacity} คน)`);
      });
