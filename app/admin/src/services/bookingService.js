...
const bookingService = {
  ...
  async getGuestDataStatus(bookingId) {
    try {
      ...
      console.error('❌ Failed to get guest data status:', error);
      throw error;
    }
  },
  ...
  async calculateDynamicPrice(roomTypeId, checkInDate, checkOutDate, adults = 1, children = 0) {
    try {
      ...
      const leadTimeDays = Math.max(0, Math.ceil((checkIn - today) / (1000 * 60 * 60 * 24)));
      ...
      const response = await apiService.post('/pricing/calculate', {
        ...
        checkInDate,
        checkOutDate,
        numAdults: parseInt(adults) || 1, //  Fixed parameter name
        numChildren: parseInt(children) || 0, //  Fixed parameter name
        leadTimeDays,
        nights
      });
      ...
      if (response && response.data) {
        ...
        return {
          ...
          data: {
            ...
            finalRate: apiData.finalRate,
            totalPrice: apiData.totalPrice, // API sends totalPrice
            totalAmount: apiData.totalPrice, // Frontend expects totalAmount
            nights: apiData.nights,
            appliedRule: apiData.appliedRule,
            occupancyPercent: apiData.occupancyPercent,
            calculation: apiData.calculation,
            holidayInfo: apiData.holidayInfo,
            breakdown: {
              roomRate: apiData.totalPrice,
              taxes: 0,
              fees: 0,
              discounts: apiData.baseRate * apiData.nights - apiData.totalPrice
            }
          }
        };
      }
  ...
  async createBooking(bookingData) {
    try {
      ...
      const apiData = {
        guestData: {
          name: guestFullName || 'Guest User', // Fallback if both names are empty
